...
export function PerformanceForm({ initialData, action, submitLabel = "Uložit" }: PerformanceFormProps) {
    ...
    const [manualSlug, setManualSlug] = useState(initialData?.url_slug || "");
    const [isSlugManual, setIsSlugManual] = useState(false);
    const slug = isSlugManual ? manualSlug : name ? slugify(name, { lower: true, strict: true }) : "";
    const [performanceState, setPerformanceState] = useState<string>(initialData?.state || "draft");
        ...
        <form action={formAction} className="space-y-6" aria-label="Formulář představení">
            ...
            <div>
                ...
                <Input
                    ...
                    required
                />
                {state?.fieldErrors?.date && <p className="mt-1 text-sm text-red-600">{state.fieldErrors.date[0]}</p>}
            </div>
            ...
            <div>
                ...
                {state?.fieldErrors?.intro_text && (
                    <p className="mt-1 text-sm text-red-600">{state.fieldErrors.intro_text[0]}</p>
                )}
            </div>
            ...
            <div>
                ...
                <div className="flex gap-2">
                    ...
                    {!isSlugManual && (
                        <Button type="button" variant="outline" onClick={() => setIsSlugManual(true)}>
                            Upravit
                        </Button>
                    )}
                </div>
                ...
                {state?.fieldErrors?.url_slug && (
                    <p className="mt-1 text-sm text-red-600">{state.fieldErrors.url_slug[0]}</p>
                )}
            </div>
            ...
            <div>
                ...
                <Select name="state" value={performanceState} onValueChange={setPerformanceState}>
                    <SelectTrigger>
                ...
                {state?.fieldErrors?.state && <p className="mt-1 text-sm text-red-600">{state.fieldErrors.state[0]}</p>}
            </div>

            {/* Success Message */}
            {state?.successMessage && (
                <div role="status" aria-live="polite" className="rounded border border-green-800 bg-green-950 p-4">
